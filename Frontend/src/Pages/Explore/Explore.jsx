...
const Explore = () => {
  ...
  useEffect(() => {
    ...
    // [ADDED] Initialize socket connection
    const socket = io(url);

    // [ADDED] Listen for real-time new service event
    socket.on("new-service", (newService) => {
        setFilteredServices((prev) => [newService, ...prev]);
        setAllServices((prev) => [newService, ...prev]);
      });

    // Listen for service removed
    socket.on("service-removed", (id) => {
      setFilteredServices((prev) => prev.filter((item) => item._id !== id));
      setAllServices((prev) => prev.filter((item) => item._id !== id));
    });

    // [ADDED] Clean up socket connection on unmount
    return () => {
      socket.disconnect();
    };
  }, [url]);
